...
def denoise(model, noisy_samples, sigma, latents=None):
    # Denoise samples
    with torch.no_grad():
        with torch.autocast(device_type="cuda" if torch.cuda.is_available() else "mps" if torch.mps.is_available() else "cpu", dtype=torch.float16, enabled=mixed_precision):
