...
def decode_to_representation(model, latents, diffusion_steps=1, device="cuda" if torch.cuda.is_available() else "mps" if torch.mps.is_available() else "cpu"):
...
@torch.no_grad()
def encode_audio_inference(audio_path, trainer, max_waveform_length_encode, max_batch_size_encode, device="cuda" if torch.cuda.is_available() else "mps" if torch.mps.is_available() else "cpu", extract_features=False):
    ...
    if repr_encoder.shape[0] > max_batch_size:
        ...
        for i in range(len(repr_encoder_ls)):
            with torch.autocast(device_type="cuda" if torch.cuda.is_available() else "mps" if torch.mps.is_available() else "cpu", dtype=torch.float16, enabled=mixed_precision): # disable float16 for encoding (can cause nans)
...
@torch.no_grad()
def decode_latent_inference(latent, trainer, max_waveform_length_decode, max_batch_size_decode, diffusion_steps=1, device="cuda" if torch.cuda.is_available() else "mps" if torch.mps.is_available() else "cpu"):
    trainer.gen = trainer.gen.to(device)
    trainer.gen.eval()
    downscaling_factor = 2**freq_downsample_list.count(0)
