...
def boominess_calculate(loudspec):
    ...
    logf = np.log10(f + 1)  # get the log version of estimated frequencies
...
def timbral_booming(fname, fs=0, dev_output=False, phase_correction=False, clip_output=False, mean=False):
    """
    ...
    rms_boom = windowed_booming #* (windowed_rms * windowed_rms)
    # get the weighted average
    if mean:
        rms_boom = np.average(windowed_booming, weights=(windowed_rms * windowed_rms))
    rms_boom = np.log10(rms_boom + 1)

    if dev_output:
        return [rms_boom, ll]
    else:
        # perform thye linear regression
        all_metrics = np.ones((3, rms_boom.shape[0]))
        all_metrics[0, :] = rms_boom
        all_metrics[1, :] = np.repeat(ll, rms_boom.shape[0])

        coefficients = np.array([43.67402696195865, -10.90054738389845, 26.836530575185435])[:, np.newaxis]

        boominess = np.sum(all_metrics * coefficients, axis=0)
